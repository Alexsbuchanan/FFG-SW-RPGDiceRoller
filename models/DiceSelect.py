...
from utils.dice_util import get_dice_from_emoji_string, roll_dice, generate_dice_rolling_string, calculate_score, \
    generate_score_string, generate_roll_string


class DiceNumberEntry(discord.ui.TextInput):
    def __init__(self, num: int):
        super().__init__(label="Number of Dice", placeholder="Num Dice", row=num)
...
class DiceSubmitButton(discord.ui.Button):
    def __init__(self, num: int):
        super().__init__(style=discord.ButtonStyle.green, label="Roll!", row=num, disabled=False)
...
class DiceIncrementButton(discord.ui.Button):
    def __init__(self, num: int):
        super().__init__(style=discord.ButtonStyle.green, emoji="➕", row=num, disabled=True)
    ...
    async def callback(self, interaction: Interaction) -> Any:
        ...
        view.count += 1
        await interaction.response.edit_message(
            content=f"{interaction.message.content}{Die(current_state.values[0]).get_emoji_gif()}", view=view)


class DiceDecrementButton(discord.ui.Button):
    def __init__(self, num: int):
        super().__init__(style=discord.ButtonStyle.red, emoji="➖", row=num, disabled=True)
...
class EphemeralRoller(discord.ui.View):
    ...
    async def receive(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(view=DiceSelectDropDown(self.ctx), ephemeral=True)
